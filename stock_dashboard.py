...
def display_summary_table_all(symbols_dict, analyzer, period):
    ...
    with st.spinner("⏳ Building summary table for all stocks... Please wait."):
        ...
        # Header
        table_html += "<tr style='background-color:#222; color:#fff;'>"
        for h in headers:
            table_html += f"<th style='padding:6px; border:1px solid #333;'>{h}</th>"
        table_html += "</tr>"
        # Rows
        for row in rows:
            ...
            try:
                if confidence_str != "N/A":
                    conf_val = float(confidence_str.strip('%'))  # e.g. "87.5%"
                    if conf_val > 85:
                        conf_style = "color:#00ff88; font-weight:bold;"
            except Exception:
                conf_style = ""
...
def main():
    ...
    # --- ONLY SHOW SUMMARY TABLE IF "ALL" IS SELECTED ---
    if symbol == 'ALL':
        #with st.expander("📋 Show All Popular Stocks (Current & Predicted Price)", expanded=True):
        display_summary_table_all(popular_stocks, analyzer, period)
        st.stop()
    # --- END ONLY SHOW SUMMARY TABLE IF "ALL" IS SELECTED ---
    ...
    # --- NEW: Summary Table at the Top ---
    latest_price = data['Close'].iloc[-1]
    # Train model and get prediction for summary table
    model_info = analyzer.train_prediction_model(data)
    prediction = analyzer.predict_next_price(model_info) if model_info else None
    # --- END NEW ---
